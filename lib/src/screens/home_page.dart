...
import 'package:taxi_app/src/utils/constant.dart';
...
class _HomePage extends State<HomePage> {
    ...
    final GoogleMapPolyline _googleMapPolyline = GoogleMapPolyline(apiKey: keyGoogle);
  ...
  void _addMarker(String mkId, PlaceItemRes place)  {
    _markerSet.add(Marker(
      ...
      icon: mkId == "from_address" ? BitmapDescriptor.defaultMarker : BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen),
      infoWindow: InfoWindow(title: place.name),
    )
    );


    print('xxxxxxxxxxxxxxxxxxxxxxx: $_markerSet');
    setState(() {
    ...
    Future<void> _moveCamera() async {
      if(_markerSet.isEmpty) {
        ...
        print("move camera123not null 22222222222222222222222222222222222222222222222");
      }
    }
  ...
  void _addMarkerQAZ(String mkId, PlaceItemRes place) {
   ...
   _checkDrawPolyline()  {
      ...
      if(_markersQAZ.length > 1) {
        ...
        List<StepsRes> stepRes = value;
        List<LatLng> stepPaths = [];
        for (var element in stepRes) {
